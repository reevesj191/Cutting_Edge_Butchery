...
```{r fig2, fig.cap="Cumulative frequencies of archaeological types according to selection stage. (1) Produced but not considered. (2) Set aside for further consideration. (3) Selected for use during the butchery task. (4) Used during butchery tasks. \\label{CE_f2}"}

cols <- c("#F21A00","#78B7C5","#3B9AB2", "#EBCC2A", "#E1AF00")

f2 <- ggplot(lithics, aes(x = Context, group = Type, fill = Type)) + 
...
#ggsave("../Figures/CE_fig_2.pdf", plot = f2, width = 7, height = 7, units = "in")

```

## Figure 3

```{r fig3, fig.cap= "Rainfall plots showing the distribution of cutting edge length (left) and mass (right) mass values for each stage of selection. In both cases, there is selection against the lithics with the shortest cutting edge and least mass at each selection stage. \\label{CE_f3}"}
...
f3 <- ggpubr::ggarrange(b,a)

ggsave("Figures/CE_fig_3.pdf", plot = f3, width = 7, height = 4, units = "in")
...
f4 <- ggarrange(plotlist = plots[],ncol = 3, 
...
#ggsave("Figures/CE_fig_4.pdf", plot = f4, width = 10, height = 4, units = "in")
...
f5 <- ggarrange(c_plot, thick_plot, edge_plot,ncol = 3, common.legend = TRUE, legend = "bottom")

#ggsave("Figures/CE_fig_5.pdf", plot = f5, width = 10, height = 4, units = "in")
...
```{r read_model}
indv.model <- readRDS("Models/CE_Participant_Model_2.RDS")
```
