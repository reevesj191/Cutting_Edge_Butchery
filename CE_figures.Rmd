---
title: "Modern stone tool users from Northern Kenya emphasize mass and edge length in the selection of cutting tools"
...
xdata <- read.csv("Data/CE_cutting_edges_2.csv")

global_model <- readRDS("Models/CE_Global_Model_4.RDS")
nsamples = 15000
```
...
min_val <- min(xdata$mean_angle_deg)
max_val <- max(xdata$mean_angle_deg[xdata$mean_angle_deg < 35])
...
```{r read_model}
indv.model <- readRDS("Models/CE_Participant_Model_4.RDS")
n_samples = 4000
```
...
max_val <- max_val <- max(xdata$mean_angle_deg[xdata$mean_angle_deg < 35])
